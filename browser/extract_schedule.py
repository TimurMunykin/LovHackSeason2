...
Usage:
    ...
    python extract_schedule.py --preprocess-stdin < page.html   # minimize+trim → stdout (for Node bridge)
"""
...
MODEL = os.environ.get("OPENAI_MODEL_SCHEDULE_EXTRACTOR", "gpt-5.4-nano")
...
def main_preprocess_stdin() -> None:
    """Read raw HTML from stdin; write minimize_html + trim_to_schedule to stdout (UTF-8)."""
    html = sys.stdin.buffer.read().decode("utf-8", errors="replace")
    sys.stdout.write(trim_to_schedule(minimize_html(html)))


if __name__ == "__main__":
    if len(sys.argv) >= 2 and sys.argv[1] == "--preprocess-stdin":
        main_preprocess_stdin()
    else:
        main()
